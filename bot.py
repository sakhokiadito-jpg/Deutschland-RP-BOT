...
REG_LINK_MSG = "❌ Зарегистрируйся в Личных Сообщениях бота @Deutechlandrpbot"


class PrivateChatRestriction(BaseMiddleware):
    """Блокирует все личные сообщения (DM) кроме /start для не-админов."""
    async def __call__(
        self,
...
JOBS = {
    "Механик": 2_000,
    "Работник кафе": 2_000,
    "Кассир (Lidl / Aldi)": 2_000,
    "Автомаляр": 2_050,
    "Заправщик АЗС": 2_050,
    "Мойщик автомобилей": 2_100,
    "Садовник / Фермер": 2_100,
    "Курьер (DHL / Amazon)": 2_150,
    "Строитель": 2_150,
    "Инкассатор": 2_200,
    "Таксист (Эконом)": 2_200,
    "Старший механик": 2_250,
    "Водитель автобуса": 2_300,
    "Шеф-повар": 2_300,
    "Таксист (Бизнес-класс)": 2_350,
    "Эвакуаторщик": 2_400,
    "Электрик": 2_450,
    "Дальнобойщик (LKW)": 2_500,
}

GOV_JOBS = {
    "Государственный служащий": 3_200,
    "Секретарь министерства": 3_500,
    "Советник министерства": 3_900,
    "Заместитель министра": 4_300,
    "Бургомистр (Мэр)": 4_700,
    "Министр обороны": 5_000,
    "Министр МВД": 5_000,
    "Министр здравоохранения": 5_000,
    "Вице-канцлер": 5_300,
    "Федеральный канцлер": 5_500,
    "Помощник прокурора": 3_200,
    "Прокурор": 3_500,
    "Адвокат": 3_600,
    "Старший прокурор": 3_900,
    "Прокурор района": 4_300,
    "Прокурор города": 4_700,
    "Судья": 5_200,
    "Генеральный прокурор": 5_500,
    "Курсант полиции": 3_200,
    "Полицейский": 3_500,
    "Старший полицейский": 3_800,
    "Младший комиссар полиции": 4_100,
    "Комиссар полиции": 4_500,
    "Старший комиссар полиции": 4_800,
    "Главный комиссар полиции": 5_100,
    "Заместитель шефа полиции": 5_300,
    "Шеф полиции": 5_500,
    "Стажёр BKA": 3_200,
    "Агент BKA": 3_500,
    "Младший следователь BKA": 3_900,
    "Следователь BKA": 4_300,
    "Старший следователь BKA": 4_700,
    "Руководитель группы BKA": 5_000,
    "Заместитель директора BKA": 5_300,
    "Директор BKA": 5_500,
    "Курсант GSG9": 3_200,
    "Оперативник GSG9": 3_600,
    "Младший оперативник GSG9": 4_000,
    "Старший оперативник GSG9": 4_500,
    "Инструктор GSG9": 4_900,
    "Командир группы GSG9": 5_200,
    "Заместитель главы GSG9": 5_400,
    "Глава GSG9": 5_500,
    "Рядовой": 3_000,
    "Ефрейтор": 3_300,
    "Младший сержант": 3_700,
    "Лейтенант": 4_100,
    "Капитан": 4_500,
    "Майор": 4_900,
    "Подполковник": 5_200,
    ...
    "Генерал": 5_500,
    "Парамедик": 3_200,
    "Пожарный (Feuerwehr)": 3_400,
    "Фельдшер": 3_600,
    "Врач неотложной помощи": 4_000,
    "Терапевт": 4_400,
    "Хирург": 4_800,
    "Старший врач": 5_100,
    "Заместитель главного врача": 5_300,
    "Главный врач": 5_500,
}
...
BUSINESSES = {
    1:  ("Мотель Grand Rock (Берлин)", 250_000, 3_500),
    2:  ("Central Gas Station", 280_000, 3_800),
    3:  ("Central Service Center (СТО+Мойка)", 300_000, 4_000),
    4:  ("Central SuperMarket", 1_500_000, 10_500),
    5:  ("McDonald's Teddy Donuts & Burgers", 500_000, 6_500),
    6:  ("Teddy Donuts & Burgers (Munich)", 750_000, 7_500),
    7:  ("АЗК WAVERIK", 950_000, 9_500),
    8:  ("Bavaria Cargo Port", 4_500_000, 18_000),
    9:  ("CPM Service (тюнинг/СТО)", 700_000, 8_000),
    10: ("Рекламная вывеска Тип 1", 42_000, 800),
    11: ("Вендинговый аппарат Тип 1", 15_000, 300),
    12: ("Тележка с напитками Тип 1", 18_500, 350),
    13: ("Тележка с напитками Тип 2", 18_500, 350),
    14: ("Рекламная вывеска Тип 2", 42_000, 800),
    15: ("Рекламная вывеска Тип 3", 42_000, 800),
    16: ("Рекламная вывеска Тип 4", 42_000, 800),
    17: ("Вендинговый аппарат Тип 2", 15_000, 300),
    18: ("Рекламный баннер (остановка, Тип 1)", 18_500, 300),
    19: ("Minimarket Small Way", 100_000, 2_500),
    20: ("Berlin Safe-Stop (паркинг)", 300_000, 3_500),
    21: ("CPM HOTEL & RESORT", 1_800_000, 11_500),
    22: ("Großes Zentrale Kaffe", 900_000, 8_500),
    23: ("Schuppig Center (ТЦ)", 1_500_000, 10_500),
    24: ("Privat Parkplatz", 400_000, 4_500),
    25: ("Autoteile", 690_000, 6_500),
    26: ("Автозаправка-мойка (Линдау)", 400_000, 4_000),
    27: ("Mechanis PRO (СТО)", 450_000, 5_000),
    28: ("Supermarket (Бавария)", 324_000, 3_800),
    29: ("Рекламный баннер Тип 2", 300, 100),
    30: ("Рекламный баннер Тип 3", 300, 100),
    31: ("Автосервис TASSIMOV", 750_000, 9_000),
    32: ("Заправка (пригород)", 250_000, 3_800),
    33: ("Заправка (магистраль)", 380_000, 4_500),
    34: ("Заправка (элитный посёлок)", 410_000, 5_000),
    35: ("Рекламная вывеска (Бавария)", 26_000, 300),
    36: ("STREETFOOD (Бавария)", 340_000, 3_500),
    37: ("Motel MOTEL RTT (Мюнхен)", 580_000, 6_000),
    38: ("АЗС (пригород)", 370_000, 4_500),
    39: ("Berlin GROUND (паркинг)", 850_000, 8_500),
    40: ("McDonald's (Берлин)", 480_000, 6_500),
    41: ("СТО (малое)", 250_000, 3_500),
    42: ("Заправка (Мюнхен)", 395_000, 5_000),
    43: ("Шиномонтаж на колёсах Тип 1", 65_000, 1_200),
    44: ("Шиномонтаж на колёсах Тип 2", 65_000, 1_200),
    45: ("Шиномонтаж на колёсах Тип 3", 65_000, 1_200),
    46: ("Шиномонтаж на колёсах Тип 4", 65_000, 1_200),
    47: ("СТО", 460_000, 6_000),
    48: ("АЗС", 450_000, 6_500),
}
    ...
    while True:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                for sym, coinbase_id in [("BTC", "BTC"), ("ETH", "ETH")]:
                    try:
                        async with session.get(
                            ...
                            mn, mx = CRYPTO_RANGES[sym]
                            CRYPTO_PRICES[sym] = max(mn, min(mx, price))
                    except Exception:
                        pass
                ...
                try:
                    async with session.get(
                        ...
                        data = await resp.json()
                        price = float(data["pepe"]["usd"])
                        mn, mx = CRYPTO_RANGES["PEPE"]
                        CRYPTO_PRICES["PEPE"] = max(mn, min(mx, price))
                except Exception:
                    pass
        except Exception:
            pass
        for sym in ["TON", "HAU", "DSC"]:
            mn, mx = CRYPTO_RANGES[sym]
            change = random.uniform(-0.08, 0.08)
            CRYPTO_PRICES[sym] = max(mn, min(mx, CRYPTO_PRICES[sym] * (1 + change)))
...
async def info_cmd(message: types.Message):
    ...
    if status == "not_registered":
        await message.answer(REG_LINK_MSG)
        return
    if status == "banned":
...
async def buy_car(message: types.Message):
    ...
    status = check_user(user)
    if status != "ok":
        await message.answer(REG_LINK_MSG if status == "not_registered" else "⛔ Вы заблокированы.")
        return
    try:
    ...
    if len(current_cars) >= garage_slots:
        await message.answer(
            ...
            f"Для покупки дополнительного слота: @ov1rlord"
        )
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[[
...
async def buy_business(message: types.Message):
    ...
    if owner:
        ...
        return
    uid = message.from_user.id
    owned_biz = db.get_businesses(uid)
    biz_slots = db.get_biz_slots(uid)
    if len(owned_biz) >= biz_slots:
        await message.answer(
            f"❌ Достигнут лимит бизнесов ({len(owned_biz)}/{biz_slots} слотов).\n"
            f"Для покупки дополнительного слота: @ov1rlord"
        )
        return
    balance = user[4]
...
async def buy_apt(message: types.Message):
    ...
    uid = message.from_user.id
    owned_apt = db.get_apartments_full(uid)
    apt_slots = db.get_apt_slots(uid)
    if len(owned_apt) >= apt_slots:
        await message.answer(
            f"❌ Достигнут лимит недвижимости ({len(owned_apt)}/{apt_slots} слотов).\n"
            f"Для покупки дополнительного слота: @ov1rlord"
        )
        return
    balance = user[4]
...
async def confirm_buy_biz(callback: types.CallbackQuery):
    ...
    if len(owned_biz) >= biz_slots:
        await callback.answer(f"❌ Лимит бизнесов ({len(owned_biz)}/{biz_slots}). Для доп. слота: @ov1rlord", show_alert=True)
        return
    balance = user[4]
    if balance < biz_price:
...
async def confirm_buy_apt(callback: types.CallbackQuery):
    ...
    if len(owned_apt) >= apt_slots:
        await callback.answer(f"❌ Лимит недвижимости ({len(owned_apt)}/{apt_slots}). Для доп. слота: @ov1rlord", show_alert=True)
        return
    balance = user[4]
    if balance < apt_price:
...
def _bank_text(uid: int) -> str:
    ...
    return (
        ...
        f"🏛 Налоговый долг: <b>{fmt(debt) if debt > 0 else '0€'}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━"
    )
...
def _tax_menu_kb(uid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
...
def _tax_text(uid: int) -> str:
    ...
    if last_ts:
        last_str = datetime.fromtimestamp(last_ts).strftime("%d.%m %H:%M")
    else:
        last_str = "—"
    return (
        f"🏛 <b>Налоги — Deutschland RP</b>\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"📋 Налоговая ставка: <b>35%</b> от дохода бизнеса\n"
        f"💸 Текущий долг: <b>{debt_str}</b>\n"
        f"🕐 Последнее начисление: <b>{last_str}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"Налог начисляется при каждой выдаче дохода бизнеса.\n"
        f"Используйте кнопку ниже для оплаты."
    )
...
async def org_rename_cmd(message: types.Message):
    ...
    if org_key not in ORG_KEYWORDS:
        await message.answer("❌ Доступно: ф1, футбол, семья, гсг9, бка")
        return
...
async def org_roster_cmd(message: types.Message):
    parts = message.text.strip().split()
    if len(parts) < 2:
        await message.answer("❌ Формат: состав ф1 / состав гсг9 / состав бка / ...")
        return
    org_key = parts[1].lower()
    if org_key not in ORG_KEYWORDS:
        await message.answer("❌ Неизвестная организация. Доступно: ф1, футбол, семья, гсг9, бка")
        return
...
async def player_create_org(message: types.Message):
    ...
    if not name:
        await message.answer("❌ Формат: создать орг [название]")
        return
    if len(name) > ORG_NAME_MAX_LEN:
        await message.answer(f"❌ Название не должно превышать {ORG_NAME_MAX_LEN} символов.")
        return
    owned = db.get_orgs_owned_by(message.from_user.id)
    if len(owned) >= MAX_PLAYER_ORGS_OWNED:
        await message.answer(
            f"❌ Вы уже являетесь лидером организации «{owned[0]['name']}».\n"
            f"Расформируйте её, чтобы создать новую: расформировать [название]"
        )
        return
    ...
    await message.answer(
        ...
        f"🏛️ <b>{name}</b>\n"
        f"👑 Вы — лидер.\n\n"
        f"Приглашайте игроков: <code>пригласить @ник {name}</code>\n"
        f"Состав: <code>орг {name}</code>",
        parse_mode="HTML"
    )
...
async def player_invite_to_org(message: types.Message):
    ...
    if target_str.startswith("@"):
        target = db.get_user_by_username(target_str[1:])
    else:
        try:
            target = db.get_user(int(target_str))
        except Exception:
            target = _resolve_user_by_nick(target_str)
    if not target:
    ...
    if org["owner_uid"] != message.from_user.id:
        await message.answer("❌ Только лидер может приглашать участников.")
        return
    ...
    try:
        await bot.send_message(
            ...
            f"{org['icon']} <b>{org['name']}</b>\n"
            f"Посмотрите состав: <code>орг {org['name']}</code>",
            parse_mode="HTML"
        )
    except Exception:
        pass
...
@dp.message(lambda m: m.text and m.text.lower().startswith("исключить "))
async def player_kick_from_org(message: types.Message):
    user = db.get_user(message.from_user.id)
    if not user:
        return
    parts = message.text.strip().split(maxsplit=2)
    if len(parts) < 3:
        await message.answer("❌ Формат: исключить @ник [название орги]")
        return
    target_str, org_name_raw = parts[1], parts[2].strip()
    if target_str.startswith("@"):
        target = db.get_user_by_username(target_str[1:])
    ...
    if org["owner_uid"] != message.from_user.id and message.from_user.id not in config.ADMIN_IDS:
        await message.answer("❌ Только лидер может исключать участников.")
        return
    if target[0] == org["owner_uid"]:
        await message.answer("❌ Нельзя исключить лидера. Расформируйте организацию.")
        return
    db.remove_player_org_member(org["org_id"], target[0])
    t_name = f"@{target[1]}" if target[1] else str(target[0])
    await message.answer(f"✅ {t_name} исключён(а) из «{org['name']}»")
    try:
        await bot.send_message(target[0], f"ℹ️ Вас исключили из организации {org['icon']} <b>{org['name']}</b>.", parse_mode="HTML")
    except Exception:
        pass


@dp.message(lambda m: m.text and m.text.lower().startswith("покинуть "))
async def player_leave_org(message: types.Message):
    user = db.get_user(message.from_user.id)
    if not user:
        return
    org_name_raw = message.text.strip()[len("покинуть "):].strip()
    org = db.get_player_org_by_name(org_name_raw)
    if not org:
        await message.answer(f"❌ Организация «{org_name_raw}» не найдена.")
        return
    if org["owner_uid"] == message.from_user.id:
        await message.answer("❌ Лидер не может просто покинуть орг. Используйте: расформировать [название]")
        return
    if not db.is_player_org_member(org["org_id"], message.from_user.id):
        await message.answer("❌ Вы не состоите в этой организации.")
        return
    db.remove_player_org_member(org["org_id"], message.from_user.id)
    await message.answer(f"✅ Вы покинули организацию «{org['icon']} {org['name']}».")
...
async def player_disband_org(message: types.Message):
    ...
    if not org:
        ...
        return
    if org["owner_uid"] != message.from_user.id and message.from_user.id not in config.ADMIN_IDS:
        await message.answer("❌ Только лидер может расформировать организацию.")
        return
    name = org["name"]
    db.delete_player_org(org["org_id"])
    await message.answer(f"✅ Организация «{name}» расформирована.")
...
@dp.message(lambda m: m.text and m.text.lower().strip() == "мои орги")
async def player_my_orgs(message: types.Message):
    user = db.get_user(message.from_user.id)
    if not user:
        await message.answer(REG_LINK_MSG)
        return
    uid = message.from_user.id
    admin_orgs = db.get_user_orgs(uid)
    p_orgs = db.get_player_orgs_for_user(uid)
    if not admin_orgs and not p_orgs:
        await message.answer("🏛️ Вы ни в одной организации не состоите.")
        return
    parts = []
    for org_key, is_owner in admin_orgs:
        parts.append(_render_org_card(org_key))
    for po in p_orgs:
        parts.append(_render_player_org_card(po["org_id"]))
    text = "🏛️ <b>Ваши организации</b>\n\n" + "\n\n━━━━━━━━━━━━━━━━\n\n".join(parts)
...
async def admin_help(message: types.Message):
    ...
    await message.answer(
        ...
        "/broadcast [текст]\n\n"
        "━━━━ 🏛️ Организации ━━━━\n"
        "ф1/футбол/семья/гсг9/бка @юз [владелец] [название]\n"
        "имя ф1/футбол/семья/гсг9/бка [название]\n"
        "убрать ф1/футбол/семья/гсг9/бка @юз\n"
        "состав ф1/футбол/семья/гсг9/бка\n\n"
...
async def admin_reset_user(message: types.Message):
    ...
    parts = message.text.strip().split()
    if len(parts) < 2:
        await message.answer("❌ Формат: /reset @ник  или  reset @ник")
        return
    target = _resolve_user_by_nick(parts[1])
    if not target:
        await message.answer("❌ Игрок не найден")
    ...
    await message.answer(
        ...
        f"Деньги, банк, крипта, транспорт, бизнесы, квартиры, "
        f"работа, налоги, штрафы — всё обнулено.\n"
        f"Аккаунт остаётся активным."
    )
    try:
        await bot.send_message(
            target_id,
            "♻️ Deutschland RP — Ваш аккаунт был сброшен администратором.\n"
            "Все игровые данные обнулены. Аккаунт активен, напишите «инфо»."
        )
    except Exception:
        pass
...
@dp.message(lambda m: m.text and m.text.startswith("/delorgs"))
async def admin_delorgs(message: types.Message):
    if not is_admin(message.from_user.id):
        return
    parts = message.text.strip().split()
    if len(parts) < 2:
        await message.answer("❌ Формат: /delorgs @ник")
        return
    target = _resolve_user_by_nick(parts[1])
    if not target:
        await message.answer("❌ Игрок не найден")
        return
    count = db.delete_all_player_orgs_by_owner(target[0])
    await message.answer(f"✅ Удалено {count} организаций у @{target[1]}")


@dp.message(lambda m: m.text and m.text.startswith("/delorg "))
async def admin_delorg(message: types.Message):
    if not is_admin(message.from_user.id):
        return
    org_name = message.text[len("/delorg "):].strip()
    if not org_name:
        await message.answer("❌ Формат: /delorg [название]")
        return
    org = db.get_player_org_by_name(org_name)
    if not org:
        await message.answer(f"❌ Организация «{org_name}» не найдена.")
        return
    db.delete_player_org(org["org_id"])
    await message.answer(f"✅ Организация «{org_name}» удалена.")
...
async def admin_add_special_car(message: types.Message):
    ...
    db.add_log(message.from_user.id, 'specialcar', f"{target[1]}:{car_name}", 0)
...
@dp.message(lambda m: m.text and m.text.startswith("/specialplate"))
async def admin_special_plate(message: types.Message):
    if not is_admin(message.from_user.id):
        return
    parts = message.text.strip().split(maxsplit=2)
    if len(parts) < 3:
        await message.answer(
            "❌ Формат: /specialplate @ник [номер]\n"
    ...
    if not car:
        await message.answer(f"❌ У @{target[1]} нет автомобилей")
        return
    db.set_custom_plate(car[0], plate_str)
    db.add_log(message.from_user.id, 'specialplate', f"{target[1]}:{plate_str}", 0)
    await message.answer(
        f"✅ Номер <code>{plate_str}</code> установлен\n"
        f"Авто: {car[3]} → @{target[1]}",
        parse_mode="HTML"
    )
    try:
        await bot.send_message(
            target[0],
            f"🔢 Вам установили специальный номер: <code>{plate_str}</code>\n"
            f"Автомобиль: {car[3]}",
            parse_mode="HTML"
        )
    except Exception:
...
async def createpromo_cmd(message: types.Message):
    ...
    try:
        ...
        await message.answer("❌ СУММА > 0, ЛИМИТ ≥ 0 (целое число)")
        return
...
@dp.message(lambda m: m.text and m.text.lower().strip().startswith("promo "))
...
async def main():
    ...
    dp.message.middleware(PrivateChatRestriction())
